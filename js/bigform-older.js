...
function getParentWithObligatory(elt) {
	for (var i=0; i<9; i++) {
...
function addHighlights() {
	...
	var elements = page.getElements( ".obligatory INPUT" );
	for (elt in elements) {
		if (!elt.getValue()) {
			var e=getParentWithObligatory(elt);
...
if (event.target.getValue()) removeHighlight(event.target);
}

function addHighlights(add_events) {
	var page = $(".page-" + currentPage);
	var obligatories = page.getElements( ".obligatory" );
	var count=0;
	for (x in obligatories) {
		var unanswered = true;
		var last_elt;
		var elements = e.getElements("INPUT");
		for (elt in elements) {
			if (elt.getValue()) { unanswered=false; last_elt=elt; break; }
		}
		if (unanswered) {
			var e=findObligatoryParent(elt);
			e.addClass("highlight");
			count++;
			if (add_events) for (elt in elements) {
				elt.addEvent('onchange', updateHighlight);
			}
		}
	}
	return count;
...
/**
 * removeHightlight ska köras när man klickar i svar,
 * inte när man är klar med hela sidan. Tycker jag (simon)
 */
...
if (location.href.match(/form/)) { addLoadEvent(init); }